...
public class BookStoreActivity extends AppCompatActivity {
    ...
    private EditText storeServiceNameEditText, storePriceEditText, storePreferenceEditText, whichStore, storeCusNameEditText, cartNumEditText, storeCusDateEditText, Status;
    ...
    private void submitBooking() {
        ...
        // Get status from Status EditText
        String status = Status.getText().toString().trim();

        // Validate inputs
        if (serviceName.isEmpty() || storePrice.isEmpty() || storePreference.isEmpty() || storeName.isEmpty() || customerName.isEmpty() || phoneNumber.isEmpty() || bookingDate.isEmpty() || selectedTime.isEmpty() || status.isEmpty()) {
            ...
            bookingsRef.child("status").setValue(status); // Save status
